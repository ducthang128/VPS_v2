...
class Userauth extends MY_Controller
{
    ...
    public function index()
    {
        if ($this->require_role('admin')) {
            ...
            echo $this->load->view('useruserauth/page_header', '', TRUE);

            echo '<p>You are logged in!</p>';

            echo $this->load->view('userauth/page_footer', '', TRUE);
        }
    }
     ...
     * Remember to add "userauth/optional_login_test" to the
    ...
    public function optional_login_test()
    {
        if ($this->verify_min_level(1)) {
            ...
            $page_content .= $this->load->view('userauth/login_form', '', TRUE);

        }

        echo $this->load->view('userauth/page_header', '', TRUE);
    ...
    public function login()
    {
        ...
        if( $this->uri->uri_string() == 'userauth/login')
        {
            show_404();
        }
        $html = '';
        ...
        $html .= $this->load->view('userauth/page_header', '', TRUE);
        $html .= $this->load->view('userauth/login_form', '', TRUE);
        $html .= $this->load->view('userauth/page_footer', '', TRUE);
    ...
    public function recover()
    {
                ...
                if( $user_data = $this->examples_model->get_recovery_data( $this->input->post('user_email') ) )
                {
                    ...
                    else
                    {
                        ...
                        $view_data['special_link'] = secure_anchor(
                            'userauth/recovery_verification/' . $user_data->user_id . '/' . $recovery_code,
                            secure_site_url( 'userauth/recovery_verification/' . $user_data->user_id . '/' . $recovery_code ),
                            'target ="_blank"'
                        );
        ...
        echo $this->load->view('userauth/recover_form', ( isset( $view_data ) ) ? $view_data : '', TRUE );
    ...
    public function recovery_verification( $user_id = '', $recovery_code = '' )
    {
        ...
        echo $this->load->view( 'userauth/choose_password_form', $view_data, TRUE );
